...
function parseRosterCosts(data: XmlData): RosterCosts {
  ...
  const factionName = Array.isArray(forces)
    ? forces[0]['@_catalogueName']
    : forces['@_catalogueName'];
...
function getForceSelections(force: BS_Force): Array<DetachmentRule> {
  const forceSelections: Array<DetachmentRule> = findAllByKey<BS_Force, BS_Selection>(
    force,
    'selection',
  )
    .filter(
      (selection: BS_Selection) =>
        ...
        normalizeToArray<BS_Category>(selection.categories?.category).some(
          (category) => category['@_name'] === 'Configuration',
        ),
    )
    .flatMap((selection) =>
      findAllByKey<BS_Selection, BS_Selection>(selection, 'selection').filter(
        (select) => !!select.profiles,
      ),
    )
    .map((selection) => {
      const profile = normalizeToArray<BS_Profile>(selection.profiles?.profile)[0];
      const characteristic = normalizeToArray<BS_Characteristic>(
...
function getSelections(forces: Array<BS_Force>): Array<Unit> {
  const allUnits: Array<Unit> = forces.flatMap((force, index) => {
    ...
    const selections: Array<BS_Selection> = normalizeToArray<BS_Selection>(
      force.selections?.selection,
    );
    return detachmentRules.length > 0
      ? parseUnits(
          selections,
          `${index + 1}. ${force['@_name']}: ${detachmentRules[0].title}`,
          detachmentRules[0].abilities,
        )
...
function getUnitType(unit: BS_Selection): string {
  ...
  const primary = Array.isArray(categories)
    ? categories.find((category: any) => category['@_primary'] === 'true')
    : categories;
...
function characteristicFromModel(
  model: BS_Selection,
  ...
  return unitProfiles.map((profile) => {
    ...
    const modelCharacteristic: Characteristics = {
      ...
      ws:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.WEAPON_SKILL)?.[
          '#text'
        ] ?? '-',
      bs:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.BALISTIC_SKILL)?.[
          '#text'
        ] ?? '-',
      s:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.STRENGTH)?.['#text'] ??
        '-',
      t:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.TOUGHNESS)?.['#text'] ??
        '-',
      w:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.WOUNDS)?.['#text'] ?? '-',
      a:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.ATTACKS)?.['#text'] ??
        '-',
      ld:
        characteristic.find((c) => c['@_name'] === UnitCharacteristicType.LEADERSHIP)?.['#text'] ??
        '-',
...
function parseModels(unit: BS_Selection, characteristic?: Array<Characteristics>): Array<Model> {
  ...
  if (unit['@_type'] === SelectionType.MODEL || unit['@_type'] === SelectionType.UPGRADE) {
    if (
      unit['@_type'] === SelectionType.UPGRADE &&
      !unitProfiles.some((profile) => profile['@_typeName'] === ProfileType.UNIT)
    )
      return [];
    ...
    const model: Model = {
      characteristics:
        characteristics.length > 0
          ? characteristics
          : characteristic
            ? [{ ...characteristic[0], count: unit['@_number'] }]
            : [],
      weapons: weapons,
    };
    return [model];
...
function parseUpgrades(unit: BS_Selection): Array<any> {
  let allUpgrades = [];
  const unitUpgrades = new Array(unit.selections?.selection)
    .flat(Infinity)
    .filter((upgrade: any) => upgrade['@_type'] === 'upgrade');
  const deeperUnitUpgrades = new Array(unit.selections?.selection)
    .flat(Infinity)
    .map((selection: any) =>
      'selections' in selection
        ? new Array(selection.selections.selection)
            .flat(Infinity)
            .filter((upgrade: any) => upgrade['@_type'] === 'upgrade')
        : [],
    )
...
function parseAbilities(unit: BS_Selection): Array<Ability> {
  ...
  const abilitiesFromUpgrades = parseUpgrades(unit)
    .filter(
      (upgrade: any) =>
        'profiles' in upgrade && upgrade.profiles.profile['@_typeName'] === 'Abilities',
    )
    .map((upgrade: any): Ability => {
  ...
  allAbilities = [
    ...new Map(
      profileAbilities.concat(abilitiesFromUpgrades).map((unit: any) => [unit.name, unit]),
    ).values(),
  ...
  return unitSelections
    ...
    .flatMap((upgrade) => {
      ...
      return weapons
        .map((weapon) => {
          const characteristic = normalizeToArray<BS_Characteristic>(
            weapon.characteristics.characteristic,
          );
          const newWeapon: Weapon = {
            name: weapon['@_name'],
            range:
              characteristic.find((c) => c['@_name'] === WeaponCharacteristicType.RANGE)?.[
                '#text'
              ] ?? '-',
            type:
              characteristic.find((c) => c['@_name'] === WeaponCharacteristicType.TYPE)?.[
                '#text'
              ] ?? '-',
            s:
              characteristic.find((c) => c['@_name'] === WeaponCharacteristicType.STRENGTH)?.[
                '#text'
              ] ?? '-',
            ap:
              characteristic.find(
                (c) => c['@_name'] === WeaponCharacteristicType.ARMOR_PENETRATION,
              )?.['#text'] ?? '-',
            d:
              characteristic.find((c) => c['@_name'] === WeaponCharacteristicType.DAMAGE)?.[
                '#text'
              ] ?? '-',
            ability:
              characteristic.find((c) => c['@_name'] === WeaponCharacteristicType.ABILITIES)?.[
                '#text'
              ] ?? '-',
            count: upgradeCount,
...
function parsePsyker(unit: any): Array<any> {
  const modelPsyker =
    'profiles' in unit
      ? new Array(unit.profiles.profile)
          .flat(Infinity)
          .filter((profile: any) => profile['@_typeName'] === 'Psyker')
      : [];
  const unitPsykerSelections =
    'selections' in unit ? new Array(unit.selections.selection).flat(Infinity) : [];
...
function parsePsychicPowers(unit: any, type: 'model' | 'unit' | 'upgrade'): Array<any> {
  ...
  const getPsyPowerFromSelection = (unit: any): Array<PsychicPower> => {
    ...
    let modelSelections = Array.isArray(unit.selections.selection)
      ? unit.selections.selection
      : new Array(unit.selections.selection);
  ...
  if (type === 'model') {
    ...
    const modelProfile =
      'profiles' in unit
        ? Array.isArray(unit.profiles.profile)
          ? unit.profiles.profile
          : new Array(unit.profiles.profile)
        : [];
...
function parseKeywords(unit: BS_Selection): Array<string> {
  ...
  const keywords = Array.isArray(categories)
    ? categories
        .filter((category: any) => category['@_primary'] === 'false')
        .map((category: any) => category['@_name'])
    : [];
  return keywords;
} // DONE
...
function parseUnitCosts(unit: BS_Selection): string {
  ...
  costs = Array.isArray(allCosts)
    ? allCosts.reduce((partial, value) => Number(partial) + Number(value)).toString()
    : allCosts;
      ...
      .reduce((map, curr) => {
        if (map.has(curr.name)) {
          ...
          map.set(curr.name, {
            ...map.get(curr.name),
            count: newCount,
            models: prevModels.concat(curr.models),
          });
        } else {
          map.set(curr.name, { ...curr });
        }
