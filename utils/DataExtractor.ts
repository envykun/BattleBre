...
function getForceRules(detachment: Array<ForceRos> | ForceRos): Array<Rule> {
  ...
  if (Array.isArray(detachment)) {
    const filteredDetachment: Array<ForceRos> = detachment.filter(
      (detach: ForceRos) => 'rules' in detach && detach.rules,
    );
    ...
    ruleData = Array.isArray(detachment.rules.rule)
      ? detachment.rules.rule
      : new Array(detachment.rules.rule);
  }
...
function getForceSelections(force: any): any {
  const forceSelection = findAllByKey(force, 'selection')
    .filter((selection: any) =>
      selection['@_type'] === 'upgrade' &&
      'categories' in selection &&
      Array.isArray(selection.categories.category)
        ? selection.categories.category.some(
            (category: any) => category['@_name'] === 'Configuration',
          )
        : selection.categories.category['@_name'] === 'Configuration',
    )
    .flatMap((selection: any) =>
      findAllByKey(selection, 'selection').filter((select: any) => 'profiles' in select),
    )
    .map((a: any) => {
...
function parseUnits(force: any, detachmentName: any, detachmentRules?: any): Array<Unit> {
  const typeModelData: Array<unknown> = force.filter(
    (selection: any) => selection['@_type'] === 'model',
  );
  const typeUnitData: Array<unknown> = force.filter(
    (selection: any) => selection['@_type'] === 'unit',
  );
  const typeUpgradeData: Array<unknown> = force.filter(
    (selection: any) => selection['@_type'] === 'upgrade' && 'profiles' in selection,
  );

  const allModels: Array<Unit> = typeModelData.map((model: any) =>
    createUnitFromData(model, detachmentName, 'model', detachmentRules),
  );
  const allUnits: Array<Unit> = typeUnitData.map((unit: any) =>
    createUnitFromData(unit, detachmentName, 'unit', detachmentRules),
  );
  ...
  if (type === 'model') {
    ...
    const subUnit: Array<any> = subUnits.filter(
      (selection: any) =>
        ...
        (selection['@_type'] === 'model' || selection['@_type'] === 'upgrade'),
    );
    const subUnitProfile: Array<Characteristics> = subUnit
      ...
      .flat(1);
    characteristics = [
      ...new Map(unitProfile.concat(subUnitProfile).map((unit: any) => [unit.name, unit])).values(),
    ];
  }
...
function parseWeapons(unit: any): Array<Weapon> {
  ...
  const weapons: Array<Weapon> = weaponUpgrades
    .concat(weaponUpgradesInUpgrades)
    .flatMap((weaponData: any) => {
        ...
        const profileArray: Array<Weapon> = weaponProfile
          ...
          .map((profile: any) => {
            ...
            const weapon: Weapon = {
              ...
              ability: weaponCharacteristic.find((item: any) => item['@_name'] === 'Abilities')[
                '#text'
              ],
  ...
  allWeapons = [...new Map(weapons.map((unit: any) => [unit.name, unit])).values()].sort((a, b) =>
...
function parseRosterCosts(data: XML2JsonRos): RosterCosts {
  ...
  const factionName: string = Array.isArray(forces.force)
    ? forces.force[0]['@_catalogueName']
    : forces.force['@_catalogueName'];
  const hasCabal: string | undefined = rosterCosts?.find(
    (r: CostRos) => r['@_name'] === CostNameRos.Cabal,
  )?.['@_value'];
  return {
    faction: factionName,
